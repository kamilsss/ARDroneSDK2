...
C_RESULT navdata_ihm_process( const navdata_unpacked_t* const pnd )
{
  ...
  if(!ihm_is_initialized) { return C_OK; }
  ...
  set_calibration_state((int32_t)pnd->navdata_magneto.magneto_calibration_ok);
  ...
  ihm_heading                                   = ((double) pnd->navdata_magneto.heading_fusion_unwrapped) / KIHM_MIL_F;
  ...
  ihm_CA[KIHM_CURVE_ALTITUDE].tval[4][ihm_val_idx] = ((double) pnd->navdata_kalman_pressure.est_z);
  ...
  ihm_CA[KIHM_CURVE_PRESSURE].tval[0][ihm_val_idx] = ((double) pnd->navdata_pressure_raw.Pression_meas) / 100.; // Pressure in mBar
  //ihm_CA[KIHM_CURVE_PRESSURE].tval[1][ihm_val_idx] = ((double) pnd->navdata_pressure.Pression_meas_uncompensated) / 100.; // Pressure in mBar
...
//#ifdef PC_USE_VISION
   ...
   * to the configuration structure of the stage in charge of displaying a rectangle
   * around the detected tags/roundels/towers/etc.
   * This stage is declared and defined in ihm_stages_o_gtk.
   */
...
//#endif
  ...
  ihm_set_start_button_state(ardrone_academy_navdata_get_takeoff_state());
	  ...
	  switch(pnd->navdata_demo.detection_camera_type)
	  {
		  ...
		  case CAD_TYPE_VISION:   snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting 2D tags"); break;
		  case CAD_TYPE_VISION_V2:snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting 2D tags (v2)"); break;
		  case CAD_TYPE_COCARDE:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting roundels"); break;
		  case CAD_TYPE_ORIENTED_COCARDE:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting roundels"); break;
		  case CAD_TYPE_H_COCARDE:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting H roundels"); break;
		  case CAD_TYPE_H_ORIENTED_COCARDE:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting H roundels"); break;
		  case CAD_TYPE_STRIPE:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting ground stripe (15Hz)"); break;
		  case CAD_TYPE_STRIPE_V:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting ground stripe (60Hz)"); break;
		  case CAD_TYPE_CAP:  snprintf(label_detection_state_value,sizeof(label_detection_state_value),"Detecting ManHunt Cap"); break;
                  case CAD_TYPE_TOWER_SIDE: snprintf(label_detection_state_value,sizeof (label_detection_state_value),"Detecting tower side"); break;
	          case CAD_TYPE_ORIENTED_COCARDE_BW: snprintf(label_detection_state_value,sizeof (label_detection_state_value),"Detecting black roundel"); break;
	  ...
	  if (pndvision->nb_detected==1 && ( pnd->navdata_demo.detection_camera_type==CAD_TYPE_ORIENTED_COCARDE ||  pnd->navdata_demo.detection_camera_type==CAD_TYPE_H_ORIENTED_COCARDE ||  pnd->navdata_demo.detection_camera_type==CAD_TYPE_ORIENTED_COCARDE_BW ) )
	  ...
	  else if (pndvision->nb_detected==1 && (pnd->navdata_demo.detection_camera_type==CAD_TYPE_TOWER_SIDE ))
	  {  snprintf(nbDetectedTags_label_buffer,sizeof(nbDetectedTags_label_buffer),"Found : %i (%4.2f deg - w: %i)",
			  pndvision->nb_detected,
			  pndvision->orientation_angle[0],
			  pndvision->width[0]);
